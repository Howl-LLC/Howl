...
import { ICON_CAMERA, ICON_CAMERA_OFF } from '../shared/icons.js';
...
@action({ UUID: 'com.howlpro.streamdeck.voice.camera' })
export class VoiceCameraAction extends SingletonAction<Settings> {
  ...
  override async onKeyDown(ev: KeyDownEvent<Settings>): Promise<void> {
    try {
      await executeAction('voice.camera');
    } catch {
  ...
  private async render(ev: { action: WillAppearEvent<Settings>['action'] }): Promise<void> {
    ...
    const cameraOff = voice?.cameraOff ?? true;
    const connected = voice?.connected ?? false;

    const image = await renderKey({
      icon: cameraOff ? ICON_CAMERA_OFF : ICON_CAMERA,
      bgColor: cameraOff ? COLORS.RED : COLORS.BG_DEFAULT,
      label: cameraOff ? 'Cam Off' : 'Cam On',
