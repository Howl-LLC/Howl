...
// them on demand instead of re-hosting the files in git. The app's CSS font
// stacks fall back to 'Plus Jakarta Sans' / system sans-serif when they are
// absent, so this step is best-effort and must NEVER fail the build.
//
  ...
  try {
    ...
    console.warn(
      `[fetch-fonts] ${fam.dir}: could not fetch (${err.message}). ` +
        `Falling back to Plus Jakarta Sans / system sans-serif.`,
    );
  }
}
